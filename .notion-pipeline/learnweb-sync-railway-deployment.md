...
- Architektur-Entscheidungen:
  ...
  - state.db: **OUT OF SCOPE** – temporär lokales Filesystem, bei Redeploy Datenverlust → Folge-Ticket (Railway Volume)
  ...
  - `/health` gibt 503 nach N konsekutiven Fehlern + Stale Sync Detection
  - RAILPACK_START_CMD statt Procfile (deprecated)
  - 409 Conflict bei laufendem Sync (Lock-Prüfung vor Thread-Start)
...
Endpoints:
    GET  /health         → Healthcheck (503 on failures, stale sync, or import error)
...
def health():
    healthy = _consecutive_failures < MAX_CONSECUTIVE_FAILURES and _import_error is None
    stale = False
    if _sync_start_time:
...
def webhook_sync():
    ...
    # Check if sync is already running (immediate feedback for callers)
    if _sync_lock.locked():
        return jsonify({"status": "already_running"}), 409

    # Run sync in non-daemon thread (graceful shutdown on worker restart)
    thread = threading.Thread(target=run_sync_safe, daemon=False)
    thread.start()
...
- [ ] `GET /health` meldet `"stale"` + 503 bei Sync > `MAX_SYNC_DURATION`
...
- **STOP** wenn: state.db-Persistenz-Entscheidung benötigt wird (OUT OF SCOPE → separat klären).
